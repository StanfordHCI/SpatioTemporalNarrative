	...
	function send_if_exists(file, req,res, callback){
		...
		fs.exists(file, function(exists){
			...
			fs.stat(file, function(err, stats){
				if (err) {
					console.error("Stat experienced an error:", err);
				}
...
markerMagick.createMarker = function(color, text, dim, cacheFile, callback) {
	try {
		im.convert(
			...
			if (err) {
				console.error("ImageMagick returned an error:", err);
				callback(err, undefined); 
			} else {
				callback(null, path.join(__dirname, "result_" + color + text + dim + ".png")); 
			}
		}); 
	} catch(error) {
		console.error("ImageMagick threw an error:", err);
		callback(error, null); 
	}
}